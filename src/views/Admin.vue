...
// ── Auth ──────────────────────────────────────────────────────────────────────
const ADMIN_PASSWORD = "haski2026"; // change as needed
...
onMounted(() => {
  if (sessionStorage.getItem("bh_admin") === "1") authed.value = true;
  loadCards();
});
...
const deleteConfirm = ref(null); // id to confirm delete
const editTarget = ref(null);   // card being edited inline
...
function submitAdd() {
  ...
  addProject({ ...newCard.value, yt: id });
  loadCards();
...
function saveEdit() {
  ...
  if (!id) return;
  updateProject(editTarget.value.id, { ...editTarget.value, yt: id });
  loadCards();
...
function execDelete() {
  deleteProject(deleteConfirm.value);
...
function toggleHighlight(card) {
  updateProject(card.id, { highlight: !card.highlight });
  loadCards();
  ...
  <div v-else class="min-h-screen bg-zinc-950 pb-32 text-white">
    ...
    <div class="sticky top-0 z-30 flex items-center justify-between border-b border-white/10 bg-zinc-950/95 px-6 py-4 backdrop-blur">
      ...
      <div class="flex items-center gap-3">
        ...
        <button @click="logout" class="px-4 py-2 text-xs tracking-widest text-white/30 transition hover:text-white">
          SALIR
        </button>
      </div>
    </div>
    ...
    <div class="mx-auto max-w-6xl px-6 pt-8">
      ...
      <transition name="slide-down">
        <div v-if="showAddForm" class="mb-8 border border-white/20 bg-zinc-900 p-6">
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              ...
              <input
                ...
                class="w-full border border-white/20 bg-transparent px-4 py-3 text-sm outline-none focus:border-white/50"
                placeholder="Nombre del video - Music Video"
              />
            </div>
            <div>
              <label class="mb-1 block text-xs tracking-widest text-white/50">TRABAJO REALIZADO</label>
              <select
                v-model="newCard.work"
                class="w-full border border-white/20 bg-zinc-900 px-4 py-3 text-sm outline-none focus:border-white/50"
              >
            ...
            <div class="md:col-span-2">
              ...
              <input
                v-model="newCard.yt"
                class="w-full border border-white/20 bg-transparent px-4 py-3 text-sm outline-none focus:border-white/50"
                placeholder="https://youtube.com/watch?v=XXXXXXXXXXX  ó  XXXXXXXXXXX"
              />
            </div>
          </div>
          ...
          <div class="mt-6 flex gap-3">
            <button @click="submitAdd" class="bg-white px-8 py-3 text-xs font-bold tracking-widest text-black transition hover:bg-white/80">
              GUARDAR
            </button>
            <button @click="showAddForm = false; addError = ''" class="border border-white/20 px-6 py-3 text-xs tracking-widest transition hover:border-white/50">
              CANCELAR
            </button>
          </div>
        </div>
      ...
      <div class="mb-6 flex flex-wrap items-center gap-4">
        <input
          v-model="search"
          class="flex-1 border border-white/20 bg-transparent px-4 py-2 text-sm outline-none placeholder-white/30 focus:border-white/50"
          placeholder="BUSCAR POR TÍTULO, TRABAJO O ID..."
        />
        <button
          @click="filterHighlight = !filterHighlight"
          class="border px-4 py-2 text-xs tracking-widest transition"
          :class="filterHighlight ? 'border-white bg-white text-black' : 'border-white/20 hover:border-white/50'"
        >
          ⭐ SOLO HIGHLIGHTS
        </button>
        <button
          @click="showResetConfirm = true"
          class="border border-white/10 px-4 py-2 text-xs tracking-widest text-white/30 transition hover:border-red-900/50 hover:text-red-400"
        >
          RESTAURAR LISTA
        </button>
      </div>
              ...
              >
                ...
                <td class="py-2 pr-4">
                  <a :href="ytLink(card.yt)" target="_blank" class="block">
                    <img
                      :src="ytThumb(card.yt)"
                      class="h-14 w-24 object-cover transition group-hover:opacity-80"
                      :alt="card.title"
                    />
                  </a>
                </td>
                <td class="py-3 pr-4 font-medium leading-snug">
                  {{ card.title }}
                </td>
                ...
                <td class="py-3 pr-4 text-center">
                  <button
                    ...
                    :class="card.highlight ? 'text-yellow-400' : 'text-white/15 hover:text-white/50'"
                    :title="card.highlight ? 'Quitar highlight' : 'Marcar como highlight'"
                  >★</button>
                </td>
                <td class="py-3 text-right">
                  <div class="flex items-center justify-end gap-2 opacity-0 transition group-hover:opacity-100">
                    <button
                      ...
                      class="border border-white/20 px-3 py-1 text-xs tracking-widest transition hover:border-white/50"
                    >EDITAR</button>
                    <button
                      @click="confirmDelete(card.id)"
                      class="border border-red-900/40 px-3 py-1 text-xs tracking-widest text-red-400 transition hover:border-red-500"
                    >✕</button>
                  </div>
                </td>
              </tr>
    ...
    <teleport to="body">
      <div v-if="showResetConfirm" style="position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.85);">
        <div class="border border-white/20 bg-zinc-950 p-8 text-center max-w-sm">
          ...
          <p class="mb-8 text-xs text-white/40">Se perderán todos los cambios y se restaurará la lista original. Esta acción no se puede deshacer.</p>
